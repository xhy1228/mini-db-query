...
class QueryTemplate(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    school_id = Column(Integer, ForeignKey('schools.id'), nullable=False, comment='学校ID')
    database_id = Column(Integer, ForeignKey('database_configs.id'), comment='数据库配置ID')
    category_id = Column(Integer, ForeignKey('template_categories.id'), comment='业务大类ID')
    category = Column(String(50), nullable=False, comment='业务大类编码(兼容)')
    category_name = Column(String(100), comment='业务大类名称(兼容)')
    category_icon = Column(String(20), comment='业务大类图标(兼容)')
    name = Column(String(100), nullable=False, comment='查询名称')
    description = Column(Text, comment='描述')
    sql_template = Column(Text, nullable=False, comment='SQL模板')
    fields = Column(JSON, comment='查询字段配置(兼容)')
...
class QueryLog(Base):
    ...
    error_detail = Column(JSON, comment='错误详情: 包含error_type, error_message, suggestion等')
    created_at = Column(DateTime, default=datetime.now, comment='创建时间')
